...
class RelationModel(nn.Module):
    ...
    def forward(self, inputs, lengths, hidden=None):
        ...
        if not hidden:
            h0 = torch.zeros(self.num_layers, batch_size, self.hidden_dim, device=self.device).to(self.device)
            c0 = torch.zeros(self.num_layers, batch_size, self.hidden_dim, device=self.device).to(self.device)
            hidden = (h0, c0)
